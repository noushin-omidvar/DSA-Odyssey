def urlify(s, true_length):
    ...
    space_count = s[:true_length].count(' ')  # Count the spaces within the true length of the string
    index = true_length + space_count * 2  # Calculate the new length after URL encoding

    if true_length < len(s) :
        s[true_length] = '\0'

    for i in range(true_length - 1, -1, -1):  # Iterate through the characters in reverse order
        if s[i] == ' ':
